...
<div class="space-y-4">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-xl font-bold text-gray-800">SRGBV Cases</h2>
            <p class="text-sm text-gray-500">Track and manage school-related gender-based violence cases</p>
        </div>
        <div class="flex gap-2">
            <a href="{{ route('srgbv.dashboard') }}" class="inline-flex items-center px-4 py-2 bg-white border border-gray-300 text-gray-700 text-sm font-medium rounded-md hover:bg-gray-50">
                📊 Dashboard
            </a>
            <a href="{{ route('srgbv.cases.create') }}" class="inline-flex items-center px-4 py-2 bg-red-700 text-white text-sm font-medium rounded-md hover:bg-red-800">
                + Report New Case
            </a>
        </div>
    </div>
    ...
    @else
        <div class="space-y-3">
            @foreach($cases as $case)
                <a href="{{ route('srgbv.cases.show', $case) }}" class="block bg-white rounded-lg border border-gray-200 hover:border-gray-300 hover:shadow-sm transition-all p-5">
                        ...
                        <div class="flex-1">
                            <div class="flex items-center gap-2 mb-1.5">
                                ...
                                @if($case->is_confidential)
                                    <span class="text-xs text-gray-400" title="Confidential">🔒</span>
                                @endif
                                @if($case->immediate_action_required)
                                    <span class="text-xs px-2 py-0.5 rounded-full bg-red-600 text-white animate-pulse">⚠ URGENT</span>
                                @endif
                            </div>
