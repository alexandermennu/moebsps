            ...
            <table class="w-full text-sm">
                <thead class="bg-gray-50 border-b border-gray-200">
                    <tr>
                        ...
                        <th class="text-center px-4 py-3 text-[11px] text-gray-500 uppercase tracking-wide font-medium w-16">Action</th>
                        @endif
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-100">
                    @forelse($activities as $tracked)
                        ...
                        <tr class="{{ $rowBg }} hover:bg-gray-50">
                            ...
                            @if($user->hasFullAccess())
                            <td class="px-4 py-3 text-center align-top">
                                <form method="POST" action="{{ route('tracked-activities.destroy', $tracked) }}" 
                                      onsubmit="return confirm('Remove this activity from the tracker? This cannot be undone.')">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="text-gray-400 hover:text-red-600" title="Remove from tracker">
                    ...
                    @empty
                        <tr>
                            <td colspan="{{ $user->isDivisionScoped() ? 7 : ($user->hasFullAccess() ? 9 : 8) }}" class="px-4 py-12 text-center text-sm text-gray-500">
