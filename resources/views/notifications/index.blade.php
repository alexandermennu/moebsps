    ...
    <div class="flex items-center justify-between">
        ...
        <div class="flex items-center gap-4">
            @if($notifications->where('is_read', true)->count() > 0)
                <form method="POST" action="{{ route('notifications.destroy-all-read') }}" onsubmit="return confirm('Delete all read notifications?')">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="text-sm text-red-600 hover:text-red-800">Delete all read</button>
                </form>
            @endif
                    ...
                    <div class="flex items-start gap-3">
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <form method="POST" action="{{ route('notifications.destroy', $notification) }}" 
                                  onclick="event.stopPropagation();" onsubmit="event.stopPropagation(); return confirm('Delete this notification?');">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="p-1 text-gray-400 hover:text-red-600 rounded hover:bg-gray-100" title="Delete">
