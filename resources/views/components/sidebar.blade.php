        ...
        <ul class="space-y-1 px-3">
            ...
            @php
                $canAccessSrgbv = $user->hasFullAccess()
                    ...
                    || $user->isCounselor()
                    || (in_array($user->role, [\App\Models\User::ROLE_SUPERVISOR, \App\Models\User::ROLE_COORDINATOR]) && $user->division && $user->division->code === 'CGPC');
                $openSrgbvCount = $canAccessSrgbv ? \App\Models\SrgbvCase::whereNotIn('status', ['resolved', 'closed'])->count() : 0;
            @endphp
            @if($canAccessSrgbv)
            <li class="pt-4">
                <p class="px-3 text-xs font-semibold text-slate-500 uppercase tracking-wider">SRGBV</p>
            </li>
            <li>
                <a href="{{ route('srgbv.dashboard') }}"
                   class="flex items-center gap-3 px-3 py-2 rounded-md text-sm {{ request()->routeIs('srgbv.dashboard') ? 'bg-slate-700 text-white' : 'text-slate-300 hover:bg-slate-700 hover:text-white' }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"/></svg>
                    SRGBV Dashboard
                </a>
            </li>
            <li>
                <a href="{{ route('srgbv.cases.index') }}"
                   class="flex items-center gap-3 px-3 py-2 rounded-md text-sm {{ request()->routeIs('srgbv.cases.*') ? 'bg-slate-700 text-white' : 'text-slate-300 hover:bg-slate-700 hover:text-white' }}">
                    ...
                    Cases
                    @if($openSrgbvCount > 0)
                        <span class="ml-auto bg-red-500 text-white text-xs rounded-full px-2 py-0.5">{{ $openSrgbvCount }}</span>
                    @endif
                </a>
            </li>
            @endif
