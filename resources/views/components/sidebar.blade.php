...
<aside class="fixed inset-y-0 left-0 w-64 bg-slate-800 text-white flex flex-col z-20">
    ...
    <nav class="flex-1 py-4 overflow-y-auto">
        <ul class="space-y-1 px-3">
            ...
            <li>
                <a href="{{ route('weekly-updates.consolidated') }}"
                   class="flex items-center gap-3 px-3 py-2 rounded-md text-sm pl-10 {{ request()->routeIs('weekly-updates.consolidated') ? 'bg-slate-700 text-white' : 'text-slate-400 hover:bg-slate-700 hover:text-white' }}">
            ...
            {{-- Cases Report --}}
            <li class="pt-4">
                <p class="px-3 text-xs font-semibold text-slate-500 uppercase tracking-wider">Cases</p>
            </li>
            <li>
                <a href="{{ route('cases-report') }}"
                   class="flex items-center gap-3 px-3 py-2 rounded-md text-sm {{ request()->routeIs('cases-report') || request()->routeIs('srgbv.*') ? 'bg-slate-700 text-white' : 'text-slate-300 hover:bg-slate-700 hover:text-white' }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                    Cases Report
                    @php $openSrgbvCount = \App\Models\SrgbvCase::whereNotIn('status', ['resolved', 'closed'])->count(); @endphp
                    @if($openSrgbvCount > 0)
