...
<div class="space-y-6">
    <div>
        <h2 class="text-sm font-semibold text-gray-900 uppercase tracking-wide">System Administration</h2>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <a href="{{ route('admin.users.create') }}" class="bg-white border border-gray-200 p-5 hover:border-slate-400 transition group">
            <div class="flex items-center gap-3">
                <div class="w-10 h-10 bg-slate-100 flex items-center justify-center group-hover:bg-slate-200">
        ...
        <a href="{{ route('admin.divisions.create') }}" class="bg-white border border-gray-200 p-5 hover:border-slate-400 transition group">
            <div class="flex items-center gap-3">
                <div class="w-10 h-10 bg-slate-100 flex items-center justify-center group-hover:bg-slate-200">
        ...
        <a href="{{ route('admin.settings.index') }}" class="bg-white border border-gray-200 p-5 hover:border-slate-400 transition group">
            <div class="flex items-center gap-3">
                <div class="w-10 h-10 bg-slate-100 flex items-center justify-center group-hover:bg-slate-200">
    ...
    <div class="bg-white border border-gray-200">
        <div class="px-5 py-4 border-b border-gray-200 flex items-center justify-between">
            <h3 class="text-sm font-semibold text-gray-900 uppercase tracking-wide">Recent Users</h3>
            <a href="{{ route('admin.users.index') }}" class="text-xs text-blue-700 hover:underline">View All</a>
        </div>
                ...
                <tbody class="divide-y divide-gray-100">
                    @foreach($recentUsers as $u)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-5 py-3">
                                <span class="text-[10px] px-1.5 py-0.5 bg-slate-100 text-slate-700">{{ $u->role_label }}</span>
                            </td>
                            <td class="px-5 py-3 text-center">
                                <span class="text-[10px] px-1.5 py-0.5 {{ $u->is_active ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700' }}">
