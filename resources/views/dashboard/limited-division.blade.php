...
<div class="space-y-6">
    ...
    {{-- Recent Assignments --}}
    <div class="bg-white border border-gray-200 p-5">
        <h3 class="text-sm font-semibold text-gray-900 uppercase tracking-wide mb-4">Division Assignments</h3>
        <div class="space-y-2">
            @forelse($recentActivities as $activity)
                <a href="{{ route('activities.show', $activity) }}" class="block p-3 border border-gray-100 hover:bg-gray-50">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-sm font-medium text-gray-800">{{ $activity->title }}</p>
                            <p class="text-xs text-gray-500">{{ $activity->assignee?->name ?? 'Unassigned' }} · {{ $activity->status }}</p>
                        </div>
                        <span class="text-[10px] px-1.5 py-0.5 {{ $activity->status_badge_color }}">{{ ucfirst(str_replace('_', ' ', $activity->status)) }}</span>
                    </div>
                </a>
            @empty
