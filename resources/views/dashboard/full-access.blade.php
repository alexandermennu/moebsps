...
<div class="space-y-6">
    ...
    {{-- Weekly Updates Summary by Division --}}
    <div>
        <div class="flex items-center justify-between mb-4">
            <h3 class="text-lg font-bold text-gray-800">📄 Weekly Updates Summary</h3>
            <a href="{{ route('weekly-updates.consolidated') }}" class="inline-flex items-center gap-1.5 px-4 py-2 bg-slate-800 text-white text-sm font-medium rounded-md hover:bg-slate-700">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            @foreach($divisionUpdateSummaries as $divSummary)
                <div class="bg-white rounded-lg border border-gray-200 p-5 hover:shadow-md transition">
                    <div class="flex items-center justify-between mb-3">
                        <div>
                            <h4 class="text-sm font-semibold text-gray-800">{{ $divSummary->name }}</h4>
                    ...
                    <div class="grid grid-cols-4 gap-2 mb-3">
                        <div class="text-center p-1.5 bg-gray-50 rounded">
                            <p class="text-sm font-bold text-gray-800">{{ $divSummary->total_updates_count }}</p>
                            <p class="text-[10px] text-gray-400">Total</p>
                        </div>
                        <div class="text-center p-1.5 bg-green-50 rounded">
                            <p class="text-sm font-bold text-green-700">{{ $divSummary->approved_updates_count }}</p>
                            <p class="text-[10px] text-green-500">Approved</p>
                        </div>
                        <div class="text-center p-1.5 bg-blue-50 rounded">
                            <p class="text-sm font-bold text-blue-700">{{ $divSummary->submitted_updates_count }}</p>
                            <p class="text-[10px] text-blue-500">Pending</p>
                        </div>
                        <div class="text-center p-1.5 bg-red-50 rounded">
                            <p class="text-sm font-bold text-red-700">{{ $divSummary->rejected_updates_count }}</p>
                            <p class="text-[10px] text-red-500">Rejected</p>
                        </div>
                    </div>

                    {{-- Latest update activity status breakdown --}}
                    @if($divSummary->latest_update)
                        <div class="mb-3">
                            <p class="text-xs text-gray-500 mb-1.5">Latest Report: {{ $divSummary->latest_update->week_start->format('M d') }} – {{ $divSummary->latest_update->week_end->format('M d') }}</p>
                        ...
                        <div class="flex items-center gap-2">
                            <div class="flex-1 bg-gray-100 rounded-full h-1.5">
                                <div class="h-1.5 rounded-full {{ $divSummary->submission_rate >= 70 ? 'bg-green-500' : ($divSummary->submission_rate >= 40 ? 'bg-yellow-500' : 'bg-red-500') }}" style="width: {{ $divSummary->submission_rate }}%"></div>
                            </div>
                            <span class="text-xs text-gray-500">{{ $divSummary->submission_rate }}%</span>
                        </div>
                        <p class="text-[10px] text-gray-400 mt-1">Approval rate</p>

                        <div class="mt-3 pt-3 border-t border-gray-100">
                            <a href="{{ route('weekly-updates.show', $divSummary->latest_update) }}" class="text-xs text-blue-600 hover:text-blue-800">View latest report →</a>
                        </div>
                    @else
