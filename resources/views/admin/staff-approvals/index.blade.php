...
<div class="space-y-4">
    <div class="flex items-center justify-between border-b border-gray-300 pb-4">
        <div>
            <h2 class="text-sm font-semibold text-gray-900 uppercase tracking-wide">Pending Staff Approvals</h2>
            <p class="text-sm text-gray-500">Review and approve staff members created by Division Directors</p>
        </div>
        @if($pendingStaff->total() > 0)
            <span class="inline-flex items-center px-3 py-1 bg-amber-100 text-amber-800 text-sm font-medium">
    ...
    @else
        <div class="space-y-3">
            @foreach($pendingStaff as $staff)
                <div class="bg-white border border-amber-200 p-5">
                    <div class="flex items-start justify-between">
                        ...
                        <div class="flex items-center gap-2 ml-4">
                            <a href="{{ route('admin.staff-approvals.show', $staff) }}" class="px-4 py-2 bg-gray-100 border border-gray-300 text-gray-700 text-sm font-medium hover:bg-gray-200">
                                View
                            </a>
                            <form method="POST" action="{{ route('admin.staff-approvals.approve', $staff) }}">
                                @csrf
                                <button type="submit" class="px-4 py-2 bg-green-600 text-white text-sm font-medium hover:bg-green-700"
                            ...
                            <button type="button"
                                    class="px-4 py-2 bg-red-600 text-white text-sm font-medium hover:bg-red-700"
                    ...
                    <form id="reject-form-{{ $staff->id }}" method="POST" action="{{ route('admin.staff-approvals.reject', $staff) }}" class="hidden mt-4 p-4 bg-red-50 border border-red-200">
                        @csrf
                        ...
                        <div class="flex gap-2 mt-2">
                            <button type="submit" class="px-3 py-1.5 bg-red-600 text-white text-sm hover:bg-red-700">Confirm Rejection</button>
                            <button type="button" class="px-3 py-1.5 bg-white border border-gray-300 text-sm hover:bg-gray-50"
