...
@section('title', 'Edit Counselor Profile — ' . $counselor->name)
...
<div class="max-w-6xl">
    <div class="mb-6 flex items-center gap-2 text-xs">
        ...
        <a href="{{ route('counselor-profile.show', $counselor) }}" class="text-blue-700 hover:underline">{{ $counselor->name }}</a>
        <span class="text-gray-400">/</span>
    ...
    <div class="bg-white border border-gray-200 p-5 mb-6">
        <div class="flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-sm text-gray-500">Edit this counselor's personal details, assignment information, qualifications, and credentials.</p>
                <p class="text-xs text-amber-600 mt-1 font-medium">Admin editing mode — all fields are editable.</p>
            </div>
        </div>
    </div>
    ...
    @if($counselor->counselor_profile_status === 'pending_review')
        <div class="mb-6 p-4 bg-amber-50 border border-amber-200 flex items-start gap-3">
            ...
            <div>
                ...
                <p class="text-xs text-amber-700 mt-0.5">This counselor's profile is awaiting review. You can approve or request changes from the <a href="{{ route('counselor-profile.show', $counselor) }}" class="underline font-medium">profile view page</a>.</p>
            </div>
        </div>
    @elseif($counselor->counselor_profile_status === 'changes_requested')
        <div class="mb-6 p-4 bg-red-50 border border-red-200 flex items-start gap-3">
            ...
            <div>
                ...
                <p class="text-xs text-red-700 mt-0.5">Changes were previously requested on this profile.</p>
    ...
    <form method="POST" action="{{ route('admin.counselor-profile.update', $counselor) }}">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
            ...
            <div class="bg-white border border-gray-200">
                ...
                <div class="p-5 space-y-4">
                    {{-- Name & Email (read-only — change via user account edit) --}}
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <p class="text-xs text-gray-400 mt-1">Change via <a href="{{ route('admin.users.edit', $counselor) }}" class="text-blue-600 hover:underline">User Account</a></p>
                        </div>
                        <div>
            ...
            {{-- ═══════════════════════════════════════════════════════════════════
                 SECTION 2: ASSIGNMENT DETAILS (All editable for admin)
                 ═══════════════════════════════════════════════════════════════════ --}}
            <div class="bg-white border border-gray-200">
                <div class="px-5 py-4 border-b border-gray-200 bg-gray-50">
                    <h3 class="text-sm font-semibold text-gray-900 uppercase tracking-wide flex items-center gap-2">
                    ...
                    <p class="text-xs text-gray-500 mt-1">School placement and assignment information.</p>
                </div>
                <div class="p-5 space-y-4">
                    {{-- Admin-Managed Fields (fully editable for admin) --}}
                    <div class="p-4 bg-blue-50 border border-blue-200">
                        <p class="text-xs font-semibold text-blue-800 uppercase tracking-wide mb-3">
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            <div>
                                <label for="counselor_school" class="block text-xs font-medium text-blue-800 mb-1">School of Assignment</label>
                                <input type="text" name="counselor_school" id="counselor_school"
                                       value="{{ old('counselor_school', $counselor->counselor_school) }}"
                                       placeholder="e.g. Monrovia Consolidated School"
                                       class="w-full px-3 py-2 border border-blue-300 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white">
                                @error('counselor_school') <p class="mt-1 text-xs text-red-600">{{ $message }}</p> @enderror
                            </div>
                            <div>
                                <label for="counselor_county" class="block text-xs font-medium text-blue-800 mb-1">County of Assignment</label>
                                <select name="counselor_county" id="counselor_county"
                                        class="w-full px-3 py-2 border border-blue-300 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white">
                                    <option value="">Select county...</option>
                                    @foreach(\App\Models\User::COUNTIES as $county)
                                        <option value="{{ $county }}" {{ old('counselor_county', $counselor->counselor_county) === $county ? 'selected' : '' }}>{{ $county }}</option>
                                    @endforeach
                                </select>
                                @error('counselor_county') <p class="mt-1 text-xs text-red-600">{{ $message }}</p> @enderror
                            </div>
                            <div>
                                <label for="counselor_status" class="block text-xs font-medium text-blue-800 mb-1">Current Status</label>
                                <select name="counselor_status" id="counselor_status"
                                        class="w-full px-3 py-2 border border-blue-300 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white">
                                    @foreach(\App\Models\User::COUNSELOR_STATUSES as $key => $label)
                                        <option value="{{ $key }}" {{ old('counselor_status', $counselor->counselor_status) === $key ? 'selected' : '' }}>{{ $label }}</option>
                                    @endforeach
                                </select>
                                @error('counselor_status') <p class="mt-1 text-xs text-red-600">{{ $message }}</p> @enderror
                            </div>
                            <div>
                                <label for="counselor_appointed_at" class="block text-xs font-medium text-blue-800 mb-1">Date of Appointment</label>
                                <input type="date" name="counselor_appointed_at" id="counselor_appointed_at"
                                       value="{{ old('counselor_appointed_at', $counselor->counselor_appointed_at?->format('Y-m-d')) }}"
                                       class="w-full px-3 py-2 border border-blue-300 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white">
                                @error('counselor_appointed_at') <p class="mt-1 text-xs text-red-600">{{ $message }}</p> @enderror
                            </div>
                        </div>
                    </div>
        ...
        <div class="bg-white border border-gray-200 mb-6">
            <div class="px-5 py-4 border-b border-gray-200 bg-gray-50">
                ...
                <p class="text-xs text-gray-500 mt-1">Area of specialization and experience. Qualifications are managed in the section below.</p>
            </div>
            <div class="p-5">
    ...
    <div class="bg-white border border-gray-200 mb-6">
        ...
        <div class="p-5">
            <p class="text-xs text-gray-500 mb-4">Manage this counselor's academic qualifications. Each entry requires the qualification level, institution, and year obtained.</p>
            ...
            @else
                <p class="text-sm text-gray-400 italic mb-5">No qualifications added yet. Click below to add one.</p>
            @endif
            ...
            <div>
                ...
                <div id="qual-form" class="mt-4 p-5 bg-gray-50 border border-gray-200" style="display: none;">
                    ...
                    <form method="POST" action="{{ route('admin.counselor-profile.qualifications.store', $counselor) }}" enctype="multipart/form-data">
                        @csrf
                        ...
                        <div class="mb-4 p-3 bg-white border border-gray-200">
                            ...
                            <p class="mt-1 text-xs text-gray-400">Upload a copy of the degree, diploma, or certificate for verification. PDF, JPG, PNG, DOC. Max 5MB.</p>
        ...
        <div class="p-5">
            ...
            <div>
                ...
                <div id="cert-form" class="mt-4 p-5 bg-gray-50 border border-gray-200" style="display: none;">
                    ...
                    <form method="POST" action="{{ route('admin.counselor-profile.certificates.store', $counselor) }}" enctype="multipart/form-data">
                        @csrf
                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-3">
                            <div>
    ...
    <div class="bg-white border border-gray-200 mb-6">
        ...
        <div class="p-5">
                    ...
                    @foreach($counselor->counselorDocuments as $doc)
                        <div class="py-3 flex items-center justify-between">
                            <div class="flex items-center gap-3">
                                <div class="w-9 h-9 bg-green-50 border border-green-200 flex items-center justify-center flex-shrink-0">
                                ...
                                <div>
                                    ...
                                    <div class="flex items-center gap-2 text-xs text-gray-400 mt-0.5">
                                        ...
                                        <span>&middot;</span>
                                        <a href="{{ $doc->file_url }}" target="_blank" class="text-blue-700 hover:underline">{{ $doc->file_name }}</a>
                                    </div>
                                </div>
                            </div>
            ...
            @else
                <p class="text-sm text-gray-400 italic mb-5">No documents uploaded yet.</p>
            @endif

            <div>
                <button type="button" id="toggle-doc-form" class="inline-flex items-center gap-1.5 px-4 py-2 bg-blue-700 text-white text-sm font-medium hover:bg-blue-600">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg>
                    Upload Document
                </button>

                <div id="doc-form" class="mt-4 p-5 bg-gray-50 border border-gray-200" style="display: none;">
                    <h4 class="text-xs font-semibold text-gray-700 uppercase tracking-wide mb-4">Upload Supporting Document</h4>
                    <form method="POST" action="{{ route('admin.counselor-profile.documents.upload', $counselor) }}" enctype="multipart/form-data">
                        @csrf
                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-3">
                            <div>
                            ...
                            <div>
                                <label class="block text-xs font-medium text-gray-600 mb-1">Title / Description *</label>
                                <input type="text" name="title" required placeholder="e.g. Ministry Appointment Letter"
                                       value="{{ old('title') }}"
                                       class="w-full px-3 py-2 border border-gray-300 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                        ...
                        <div class="flex items-center gap-2">
                            <button type="submit" class="px-4 py-2 bg-blue-700 text-white text-sm font-medium hover:bg-blue-600">Upload</button>
                            <button type="button" onclick="document.getElementById('doc-form').style.display='none'" class="px-4 py-2 bg-white border border-gray-300 text-gray-700 text-sm font-medium hover:bg-gray-50">Cancel</button>
                        </div>
                    </form>
                </div>
            </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const toggleCertBtn = document.getElementById('toggle-cert-form');
    ...
    const toggleDocBtn = document.getElementById('toggle-doc-form');
    const docForm  = document.getElementById('doc-form');
    toggleDocBtn.addEventListener('click', function () {
        docForm.style.display = docForm.style.display === 'none' ? 'block' : 'none';
    });
    @if($errors->hasAny(['document_type', 'title', 'document']))
