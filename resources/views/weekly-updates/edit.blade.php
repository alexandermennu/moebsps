...
<div class="max-w-6xl">
    <div class="mb-6">
        <a href="{{ route('weekly-updates.show', $weeklyUpdate) }}" class="text-xs text-blue-700 hover:underline">Back to Update</a>
    </div>

    <form method="POST" action="{{ route('weekly-updates.update', $weeklyUpdate) }}" id="weeklyUpdateForm">
        @csrf
        @method('PUT')

        {{-- Header Section --}}
        <div class="bg-white border border-gray-200 p-6 mb-6">
            <h2 class="text-sm font-semibold text-gray-900 uppercase tracking-wide border-b border-gray-200 pb-2 mb-6">Edit Weekly Update</h2>
            <p class="text-sm text-gray-500 mb-6">{{ $user->division?->name }} · Submitted by {{ $user->name }}</p>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div class="md:col-span-2">
                    ...
                    <select id="week_select" onchange="updateWeekDates()" 
                        ...
                        @php
                            $updateStart = $weeklyUpdate->week_start;
                            $updateMonth = $updateStart->month;
                            $updateYear = $updateStart->year;
                            
                            // Get weeks for 3 months: previous, current update's month, and next
                            $prevMonth = $updateStart->copy()->subMonth();
                            $nextMonth = $updateStart->copy()->addMonth();
                            
                            $prevMonthWeeks = \App\Models\WeeklyUpdate::getWeeksInMonth($prevMonth->year, $prevMonth->month);
                            $currentMonthWeeks = \App\Models\WeeklyUpdate::getWeeksInMonth($updateYear, $updateMonth);
                            $nextMonthWeeks = \App\Models\WeeklyUpdate::getWeeksInMonth($nextMonth->year, $nextMonth->month);
                            
                            // Current week from the update
                            $selectedStart = old('week_start', $weeklyUpdate->week_start->format('Y-m-d'));
                        @endphp
                        <optgroup label="{{ $prevMonth->format('F Y') }}">
                            @foreach($prevMonthWeeks as $week)
                        ...
                        <optgroup label="{{ $updateStart->format('F Y') }}">
                ...
                <input type="hidden" name="week_start" id="week_start" value="{{ old('week_start', $weeklyUpdate->week_start->format('Y-m-d')) }}">
                <input type="hidden" name="week_end" id="week_end" value="{{ old('week_end', $weeklyUpdate->week_end->format('Y-m-d')) }}">
            </div>
        ...
        <div class="bg-white border border-gray-200 p-6 mb-6">
            ...
            <div class="mb-4">
                ...
                <textarea name="support_needed" id="support_needed" rows="2"
                          class="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-slate-500"
                          placeholder="Any support or resources needed from the bureau...">{{ old('support_needed', $weeklyUpdate->support_needed) }}</textarea>
            </div>
            ...
            <div>
                ...
                <textarea name="key_metrics" id="key_metrics" rows="2"
                          class="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-slate-500"
                          placeholder="Key performance metrics or numbers for the week...">{{ old('key_metrics', $weeklyUpdate->key_metrics) }}</textarea>
            </div>
        </div>
...
<script>
    ...
    // Initialize with existing activities or old form data
    document.addEventListener('DOMContentLoaded', function() {
        ...
        @elseif($weeklyUpdate->activities->count() > 0)
            @foreach($weeklyUpdate->activities as $act)
                addActivityRow({
                    activity: @json($act->activity),
                    responsible_persons: @json($act->responsible_persons ?? ''),
            ...
            @endforeach
        @else
            {{-- Fallback: if no activities exist yet (old data), show one row --}}
            addActivityRow();
        @endif
    });
</script>
