...
<div class="max-w-3xl">
    ...
    <div class="bg-white rounded-lg border border-gray-200 p-6">
        ...
        <form method="POST" action="{{ route('weekly-updates.store') }}">
            @csrf
            ...
            <div class="mb-4">
                ...
                <textarea name="accomplishments" id="accomplishments" rows="5" required
                          class="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-slate-500"
                          placeholder="List your key accomplishments for this week...">{{ old('accomplishments') }}</textarea>
            </div>
            ...
            <div class="mb-4">
                ...
                <textarea name="challenges" id="challenges" rows="3"
                          class="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-slate-500"
                          placeholder="Any challenges or blockers encountered...">{{ old('challenges') }}</textarea>
            </div>
            ...
            <div class="mb-4">
                ...
                <textarea name="support_needed" id="support_needed" rows="3"
                          class="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-slate-500"
                          placeholder="Any support or resources needed...">{{ old('support_needed') }}</textarea>
            </div>
            ...
            <div class="mb-6">
                ...
                <textarea name="key_metrics" id="key_metrics" rows="3"
                          class="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-slate-500"
                          placeholder="Key performance metrics or numbers...">{{ old('key_metrics') }}</textarea>
            </div>

            <div class="flex gap-3">
                <button type="submit" name="status" value="submitted" class="px-4 py-2 bg-slate-800 text-white text-sm font-medium rounded-md hover:bg-slate-700">
                    Submit for Review
                </button>
                <button type="submit" name="status" value="draft" class="px-4 py-2 bg-white border border-gray-300 text-gray-700 text-sm font-medium rounded-md hover:bg-gray-50">
                    Save as Draft
                </button>
            </div>
        </form>
